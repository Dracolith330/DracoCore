package dracocore.blocks.tileentity.generic;
...
import dracocore.network.Annotations;
...
public abstract class TileEntityAdvanced extends TileEntity implements IPacketReceiver
{
    ...
    public void updateEntity()
    {
        ...
        if (this.isNetworkedTile() && this.ticks % this.getPacketCooldown() == 0)
        {
            ...
            {
                //CoreMain.PacketHandler.sendToServer(new PacketDynamic(this));
            }
            else if (!this.worldObj.isRemote && this.fieldCacheClient.size() > 0)
            {
            	//CoreMain.PacketHandler.sendToAllAround(new PacketDynamic(this), new TargetPoint(this.worldObj.provider.dimensionId, this.xCoord, this.yCoord, this.zCoord, this.getPacketRange()));
            }
        }
    }
