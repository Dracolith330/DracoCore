...
public class TileEntityBurnerManaGenerator extends TileBaseUniversalElectricalSource implements IInventory, ISidedInventory, IPacketReceiver, IConnector
{
    ...
    public static final int MAX_GENERATE_WATTS = Config.BurnerManaGeneratorManaPerSecond;
    
    public TileEntityBurnerManaGenerator()
    {
    	this(1);
    }
    
	public TileEntityBurnerManaGenerator(int tier)
    {
        this.inv = new ItemStack[1];this.storage.setMaxExtract(TileEntityBurnerManaGenerator.MAX_GENERATE_WATTS);
    	this.storage.setMaxReceive(TileEntityBurnerManaGenerator.MAX_GENERATE_WATTS);
    	if (tier == 2)
    	{
    ...
    public void updateEntity()
    {
    	...
    	if (!this.worldObj.isRemote)
    	{
    		...
    		{
    			this.generateWatts = Math.min(Math.max(this.getGenerate(), 0), TileEntityBurnerManaGenerator.MAX_GENERATE_WATTS);
    		}
    		else
    		{
