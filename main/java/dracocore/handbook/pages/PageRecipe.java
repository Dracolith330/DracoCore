...
public class PageRecipe extends LexiconPage {
    ...
    public void renderScreen(IGuiLexiconEntry gui, int mx, int my) {
        ...
        if (tooltipStack != null) {
            ...
            if (tooltipEntry) {
                VazkiiRenderHelper.renderTooltipOrange(mx, my + tooltipY, Arrays.asList(EnumChatFormatting.GRAY + StatCollector.translateToLocal("dracocoremisc.clickToRecipe")));
                tooltipY += 18;
            }

            if (tooltipContainerStack != null)
                VazkiiRenderHelper.renderTooltipGreen(mx, my + tooltipY, Arrays.asList(EnumChatFormatting.AQUA + StatCollector.translateToLocal("dracocoremisc.craftingContainer"), tooltipContainerStack.getDisplayName()));
        }
    ...
    @SideOnly(Side.CLIENT)
    public void renderItemAtGridPos(IGuiLexiconEntry gui, double x, double d, ItemStack stack, boolean accountForContainer) {
        if (stack == null || stack.getItem() == null)
            return;
        stack = stack.copy();
