...
public class PureSparkRender extends TileEntitySpecialRenderer 
{
    ...
    public PureSparkRender() 
    {
    	this.Obelisk = AdvancedModelLoader.loadModel(new ResourceLocation(References.texturelocation, "models/PureSpark.obj"));
    }
    
    
    public void renderModelAt(TilePureSpark tileentity, double d, double d1, double d2, float f)
    {
    	FMLClientHandler.instance().getClient().renderEngine.bindTexture(PureSparkRender.Wire);
    	...
    	RenderHelper.renderFacingQuad(tileentity.xCoord + 0.5D, tileentity.yCoord + 0.8D, tileentity.zCoord + 0.5D, 0.0F, 0.2F, 1, 1, 0, f);
    ...
    {
    	renderModelAt((TilePureSpark)tileentity, d, d1, d2, f);
    }
