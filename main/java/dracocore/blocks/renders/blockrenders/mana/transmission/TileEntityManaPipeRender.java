package dracocore.blocks.renders.blockrenders.mana.transmission;
...
public class TileEntityManaPipeRender implements ISimpleBlockRenderingHandler
{
    final int renderID;

    public TileEntityManaPipeRender(int var1)
    {
        this.renderID = var1;
    }
    ...
    public void renderPipe(RenderBlocks renderblocks, IBlockAccess iblockaccess, Block block, int x, int y, int z)
    {
        ...
        if (tileEntity != null)
        {
            final TileEntity[] connections = ManaUtil.getAdjacentPowerConnections(tileEntity);
